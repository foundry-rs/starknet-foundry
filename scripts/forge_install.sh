...
# This is just a little script that can be downloaded from the internet to install Forge.
# It just does platform detection, downloads the release archive, extracts it and tries to make
# the `forge` binary available in $PATH in least invasive way possible.
#
...
FORGE_REPO="https://github.com/foundry-rs/starknet-foundry"
XDG_DATA_HOME="${XDG_DATA_HOME:-"${HOME}/.local/share"}"
INSTALL_ROOT="${XDG_DATA_HOME}/forge"
LOCAL_BIN="${HOME}/.local/bin"
LOCAL_BIN_ESCAPED="\$HOME/.local/bin"

usage() {
  cat <<EOF
The installer for Forge
...
main() {
  ...
  say "installed forge to ${_installdir}"
  ...
  if echo ":$PATH:" | grep -q ":${LOCAL_BIN}:"; then
    echo "Forge has been successfully installed and should be already available in your PATH."
    echo "Run 'forge --version' to verify your installation. Happy coding!"
  else
...
resolve_version() {
  ...
  say "retrieving $_requested_version version from ${FORGE_REPO}..."
  _response=$(ensure curl -Ls -H 'Accept: application/json' "${FORGE_REPO}/releases/${_requested_ref}")
...
create_install_dir() {
  ...
  if [ -d "$_installdir" ]; then
    ...
    say "removed existing forge installation at ${_installdir}"
  fi
...
download() {
  ...
  local _tarball="forge-${_resolved_version}-${_arch}.tar.gz"
  local _url="${FORGE_REPO}/releases/download/${_resolved_version}/${_tarball}"
  local _dl="$_tempdir/forge.tar.gz"
...
create_symlink() {
  ...
  local _forge="${_installdir}/bin/forge"
  local _symlink="${LOCAL_BIN}/forge"

  ensure mkdir -p "$LOCAL_BIN"
  ensure ln -fs "$_forge" "$_symlink"
  ensure chmod u+x "$_symlink"

  say "created symlink ${_symlink} -> ${_forge}"
}
...
add_local_bin_to_path() {
  ...
  *)
  ...
  echo \
    ...
    "Run 'source ${_profile}' or start a new terminal session to use Forge."
}
