...
# This is just a little script that can be downloaded from the internet to install Cast.
# It just does platform detection, downloads the release archive, extracts it and tries to make
# the `cast` binary available in $PATH in least invasive way possible.
#
...
CAST_REPO="https://github.com/foundry-rs/starknet-foundry"
XDG_DATA_HOME="${XDG_DATA_HOME:-"${HOME}/.local/share"}"
INSTALL_ROOT="${XDG_DATA_HOME}/cast"
LOCAL_BIN="${HOME}/.local/bin"
LOCAL_BIN_ESCAPED="\$HOME/.local/bin"

usage() {
  cat <<EOF
The installer for Cast
...
Options:
  ...
  -h, --help             Print help
  -v, --version          Specify Cast version to install
...
main() {
  ...
  say "installed cast to ${_installdir}"
  ...
  if echo ":$PATH:" | grep -q ":${LOCAL_BIN}:"; then
    echo "Cast has been successfully installed and should be already available in your PATH."
    echo "Run 'cast --version' to verify your installation. Happy coding!"
  else
    ...
    echo "Then, run 'ccarb --version' to verify your installation. Happy coding!"
  fi
...
resolve_version() {
  ...
  say "retrieving $_requested_version version from ${CAST_REPO}..."
  _response=$(ensure curl -Ls -H 'Accept: application/json' "${CAST_REPO}/releases/${_requested_ref}")
...
create_install_dir() {
  ...
  if [ -d "$_installdir" ]; then
    ...
    say "removed existing cast installation at ${_installdir}"
  fi
...
download() {
  ...
  local _tarball="cast-${_resolved_version}-${_arch}.tar.gz"
  local _url="${CAST_REPO}/releases/download/${_resolved_version}/${_tarball}"
  local _dl="$_tempdir/cast.tar.gz"
...
create_symlink() {
  ...
  local _cast="${_installdir}/bin/cast"
  local _symlink="${LOCAL_BIN}/cast"

  ensure mkdir -p "$LOCAL_BIN"
  ensure ln -fs "$_cast" "$_symlink"
  ensure chmod u+x "$_symlink"

  say "created symlink ${_symlink} -> ${_cast}"
}
...
add_local_bin_to_path() {
  ...
  *)
  ...
  echo \
    ...
    "Run 'source ${_profile}' or start a new terminal session to use Cast."
}
