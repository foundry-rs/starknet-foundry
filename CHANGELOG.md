...
- Test performance has been enhanced. When using a filter, only the matching tests are compiled. The summary no longer displays the number of filtered-out tests when running all tests. When a filter is applied, it now shows `other filtered out` rather than the previous `<n> filtered out`.
