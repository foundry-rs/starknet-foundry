...
use starknet::secp256_trait::{
    Signature, recover_public_key, Secp256Trait, Secp256PointTrait, signature_from_vrs,
    is_valid_signature
};
...
impl KeyPairImpl of KeyPairTrait {
    ...
    fn from_private(private_key: u256, curve: EllipticCurve) -> KeyPair {
        ...
        let output = cheatcode::<
            'get_public_key'
        >(array![pk_low, pk_high, *serialized_curve[0]].span());
...
impl KeyPairSigner of Signer<KeyPair> {
    fn sign(ref self: KeyPair, message_hash: u256) -> Result<(u256, u256), felt252> {
        let mut serialized_curve = array![];
...
impl KeyPairVerifier of Verifier<KeyPair> {
    fn verify(ref self: KeyPair, message_hash: u256, signature: (u256, u256)) -> bool {
        let (r, s) = signature;
