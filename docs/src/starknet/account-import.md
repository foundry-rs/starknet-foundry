...
To import an account whose signing key lives on a [Ledger device](./ledger.md), pass `--ledger-path` instead of `--private-key`. The public key is read directly from the device.
