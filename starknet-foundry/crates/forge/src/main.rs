...
use rust_test_runner::scarb::{get_contracts_map, try_get_starknet_artifacts_path};
...
fn main_execution() -> Result<()> {
    ...
    for package in &scarb_metadata.workspace.members {
        let protostar_config = forge::scarb::config_from_scarb_for_package(&scarb_metadata, package)?;
        let (base_path, dependencies) = forge::scarb::dependencies_for_package(&scarb_metadata, package)?;
