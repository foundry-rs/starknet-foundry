...
pub fn reverse_transform_event(
    ...
    let selector = keys.first().copied().ok_or({
...
impl<'a> EventReverseTransformer<'a> {
    ...
    fn transform_typed_event(
        ...
        match typed_event {
            TypedAbiEvent::Struct(abi_event_struct) => {
                self.transform_event_struct(abi_event_struct, selector_source, db)
            }
            TypedAbiEvent::Enum(abi_event_enum) => {
                self.transform_event_enum(abi_event_enum, selector_source, db)
            }
        }
    }
        ...
        let fields = abi_event_struct
            ...
            .map(|member| {
                Ok(StructField {
                    ...
                    value: self.transform_event_field(member, db)?,
                })
            })
    ...
    fn transform_event_field(
        &mut self,
        field: &EventField,
        db: &SimpleParserDatabase,
    ) -> Result<Type, InternalEventError> {
        match field.kind {
            EventFieldKind::Key => self
                .transform_from_keys(&field.r#type, db)
                .map_err(Into::into),
            EventFieldKind::Data => self
                .transform_from_data(&field.r#type, db)
                .map_err(Into::into),
    ...
    fn transform_from_keys(
        &mut self,
        expr: &str,
        db: &SimpleParserDatabase,
    ) -> Result<Type, ReverseTransformEventError> {
        let mut reverse_transformer = ReverseTransformer::new(self.keys, self.abi);
        let value = reverse_transformer.parse_and_transform(expr, db)?;
        self.keys = reverse_transformer.into_remaining();
        Ok(value)
    }

    fn transform_from_data(
        &mut self,
        expr: &str,
        db: &SimpleParserDatabase,
    ) -> Result<Type, ReverseTransformEventError> {
