    ...
    fn handle_cheatcode(
        ...
        let res = match selector {
            "start_roll" => {
                ...
                Ok(CheatcodeHandlingResult::Result(vec![]))
            }
            "stop_roll" => {
            ...
            "declare" => {
                ...
                match blockifier_state.declare(&contract_name, &self.extension_state.contracts) {
                    Ok(class_hash) => {
                        ...
                        Ok(CheatcodeHandlingResult::Result(result))
                    }
            ...
            "precalculate_address" => {
                ...
                Ok(CheatcodeHandlingResult::Result(vec![felt_contract_address]))
            }
            "var" => {
                ...
                Ok(CheatcodeHandlingResult::Result(vec![parsed_env_var]))
            }
            "get_class_hash" => {
                let contract_address = inputs[0].clone().into_();
                ...
                match blockifier_state.get_class_hash(contract_address) {
                    Ok(class_hash) => {
                        ...
                        Ok(CheatcodeHandlingResult::Result(vec![felt_class_hash]))
                    }
            ...
            "l1_handler_execute" => {
                    ...
                    .result
                {
                    CallContractResult::Success { .. } => {
                        Ok(CheatcodeHandlingResult::Result(vec![Felt252::from(0)]))
                    }
                    CallContractResult::Failure(CallContractFailure::Panic { panic_data }) => Ok(
                        CheatcodeHandlingResult::Result(cheatcode_panic_result(panic_data)),
                    ),
            ...
            "read_txt" => {
                ...
                Ok(CheatcodeHandlingResult::Result(parsed_content))
            }
            "read_json" => {
                let file_path = inputs[0].clone();
            ...
            "spy_events" => {
                ...
                let id = self
                    ...
                    .cheatnet_state
                    .spy_events(spy_on);
                Ok(CheatcodeHandlingResult::Result(vec![Felt252::from(id)]))
            }
            "fetch_events" => {
                let id = &inputs[0];
            ...
            "event_name_hash" => {
                ...
                Ok(CheatcodeHandlingResult::Result(vec![Felt252::from(hash)]))
            }
            "generate_ecdsa_keys" => {
            ...
            "get_public_key" => {
                ...
                Ok(CheatcodeHandlingResult::Result(vec![key_pair
                    .verifying_key()
                    .scalar()
                    .into_()]))
            }
            ...
            _ => Ok(CheatcodeHandlingResult::Forward),
        }?;
...
fn handle_deploy_result(
    ...
    match deploy_result {
        ...
        Err(CheatcodeError::Recoverable(panic_data)) => Ok(CheatcodeHandlingResult::Result(
