...
pub fn finalize_execution(
    ...
    // region: Modified blockifier code
    copied_code::finalize_runner(runner, n_total_args, program_extra_data_length)?;
    syscall_handler
        .read_only_segments
    ...
    let vm_resources_without_inner_calls =
        copied_code::extract_vm_resources(runner, syscall_handler, tracked_resource)?;

    syscall_handler.finalize();

    let vm_resources = copied_code::total_vm_resources(
