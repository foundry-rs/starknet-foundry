...
pub struct Runner {
    sierra_casm_runner: SierraCasmRunner,
}
...
impl Runner {
    ...
    pub fn new(sierra_program: Program) -> Self {
        Runner {
            sierra_casm_runner: SierraCasmRunner::new(
                sierra_program,
    ...
    pub fn run_from_test_case(
        ...
        let runner = &self.sierra_casm_runner;
    ...
    pub fn get_test_case_args(&self, case_name: &str) -> Vec<ConcreteTypeId> {
        let function = self
            .sierra_casm_runner
            .find_function(case_name)
            .expect("Failed to find the test case.");
