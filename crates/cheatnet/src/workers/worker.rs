...
use blockifier::execution::contract_class::{
    ContractClass as ContractClassBlockifier,
    ContractClassV1,
};
...
use starknet::core::types::{
    BlockId, ContractClass as ContractClassStarknet, FieldElement,
};
...
impl Worker {
    ...
    #[must_use]
    pub fn get_nonce(&self, contract_address: ContractAddress) -> Nonce {
    ...
    pub fn get_class_hash_at(&self, contract_address: ContractAddress) -> ClassHash {
        ...
        ClassHash(StarkHash::from(class_hash))
    }
    ...
    pub fn get_storage_at(&self, contract_address: ContractAddress, key: StorageKey) -> StarkFelt {
        ...
        StarkFelt::from(value)
    }

    pub fn get_compiled_contract_class(
        &mut self,
        class_hash: &ClassHash,
    ) -> ContractClassBlockifier {
        let rt = Runtime::new().expect("Could not instantiate Runtime");
        let contract_class = rt
            .block_on(
                self.client.get_class(
                    BlockId::Tag(Latest),
                    FieldElement::from_bytes_be(&class_hash_to_felt(*class_hash).to_be_bytes())
                        .unwrap(),
                ),
            )
            .unwrap();
        ...
        match contract_class {
            ContractClassStarknet::Sierra(flattened_class) => {
                ...
                ContractClassBlockifier::V1(ContractClassV1::try_from(casm_contract_class).unwrap())
            }
            ContractClassStarknet::Legacy(_) => {
                ContractClassBlockifier::V0(Default::default())
            }
        }
    }
}
