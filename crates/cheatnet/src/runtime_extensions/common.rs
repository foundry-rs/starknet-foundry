...
pub fn get_syscalls_gas_consumed(
    ...
    syscall_usage
        ...
        .map(|(selector, usage)| {
            ...
            // `linear_factor` is relevant only for `deploy`, `meta_tx_v0` and `send_message_to_l1` syscalls, for other syscalls it is 0
            ...
            let base_cost = match selector {
                ...
                | SyscallSelector::SendMessageToL1 => syscall_gas_cost.get_syscall_cost(0),
...
pub fn get_relocated_vm_trace(cairo_runner: &mut CairoRunner) -> Vec<RelocatedTraceEntry> {
    ...
    {
        // Relocation also runs on paths where the VM never produced a trace,
        // so a missing trace is not an error here.
