...
use crate::runtime_extensions::call_to_blockifier_runtime_extension::execution::execution_utils::{get_cheated_data, update_trace_data, exit_error_call};
...
pub fn execute_call_entry_point(
    ...
    let trace_data_handled_by_revert_call =
        cheatnet_state.trace_data.is_entry_point_on_top(entry_point);
    if !trace_data_handled_by_revert_call {
        let cheated_data = get_cheated_data(entry_point, cheatnet_state);
        cheatnet_state
            .trace_data
...
pub fn non_reverting_execute_call_entry_point(
    ...
    let execution_result =
        execute_call_entry_point(entry_point, state, cheatnet_state, context, remaining_gas);
