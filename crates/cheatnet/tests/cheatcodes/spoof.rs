...
use cheatnet::{rpc::call_contract, state::{BlockifierState, CheatnetState}};
...
fn assert_all_mock_checker_getters(
    ...
    let tx_hash = call_contract_getter_by_name(blockifier_state, cheatnet_state, contract_address, "get_tx_hash");
    assert_success!(tx_hash, expected_tx_hash);
    let nonce = call_contract_getter_by_name(blockifier_state, cheatnet_state, contract_address, "get_nonce");
    assert_success!(nonce, expected_nonce);
    let account_address =
        call_contract_getter_by_name(blockifier_state, cheatnet_state, contract_address, "get_account_contract_address");
    assert_success!(account_address, expected_account_address);
    let version = call_contract_getter_by_name(blockifier_state, cheatnet_state, contract_address, "get_version");
    assert_success!(version, expected_version);
    let chain_id = call_contract_getter_by_name(blockifier_state, cheatnet_state, contract_address, "get_chain_id");
    assert_success!(chain_id, expected_chain_id);
    let max_fee = call_contract_getter_by_name(blockifier_state, cheatnet_state, contract_address, "get_max_fee");
    assert_success!(max_fee, expected_max_fee);
    let signature = call_contract_getter_by_name(blockifier_state, cheatnet_state, contract_address, "get_signature");
...
fn spoof_simple() {
    ...
    let (
        ...
        signature_before,
        _,
    ) = call_mock_checker_getters(&mut blockifier_state, &mut cheatnet_state, &contract_address);
...
fn spoof_with_other_syscall() {
    ...
    let output =
        call_contract(&mut blockifier_state, &mut cheatnet_state,&contract_address, &selector, vec![].as_slice()).unwrap();
...
fn spoof_in_constructor() {
    ...
    let class_hash = blockifier_state.declare(&contract_name, &contracts).unwrap();
    ...
    let contract_address = deploy(&mut blockifier_state, &mut cheatnet_state, &class_hash, &[]).unwrap().contract_address;
...
fn spoof_proxy() {
    ...
    let proxy_address = deploy_contract(&mut blockifier_state, &mut cheatnet_state, "SpoofCheckerProxy", vec![].as_slice());
    let proxy_selector = felt_selector_from_name("get_spoof_checkers_tx_hash");
    let output = call_contract(&mut blockifier_state, &mut cheatnet_state,
        &proxy_address,
        &proxy_selector,
...
fn spoof_library_call() {
    ...
    let lib_call_address = deploy_contract(&mut blockifier_state, &mut cheatnet_state, "SpoofCheckerLibCall", vec![].as_slice());
