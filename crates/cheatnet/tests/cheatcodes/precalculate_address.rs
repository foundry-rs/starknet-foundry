...
fn precalculate_address_simple() {
    ...
    let actual1 = deploy(&mut blockifier_state, &mut cheatnet_state, &class_hash, &[]).unwrap().contract_address;

    let precalculated2 = cheatnet_state.precalculate_address(&class_hash, &[]);
    let actual2 = deploy(&mut blockifier_state, &mut cheatnet_state, &class_hash, &[]).unwrap().contract_address;
...
fn precalculate_address_calldata() {
    ...
    let actual1 = deploy(&mut blockifier_state, &mut cheatnet_state, &class_hash, &calldata1)
        .unwrap()
        .contract_address;

    let precalculated2_post_deploy = cheatnet_state.precalculate_address(&class_hash, &calldata2);

    let actual2 = deploy(&mut blockifier_state, &mut cheatnet_state, &class_hash, &calldata2)
        .unwrap()
        .contract_address;
