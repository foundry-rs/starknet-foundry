...
fn get_class_hash_simple() {
    ...
    assert_eq!(class_hash, blockifier_state.get_class_hash(contract_address).unwrap());
}
...
fn get_class_hash_upgrade() {
    ...
    let contract_name = "HelloStarknet".to_owned().to_felt252();
    let hello_starknet_class_hash = blockifier_state.declare(&contract_name, &contracts).unwrap();
