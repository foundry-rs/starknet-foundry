...
fn mock_call_simple() {
    ...
    let contract_address = deploy_contract(&mut blockifier_state, &mut cheatnet_state, "MockChecker", &[Felt252::from(420)]);
...
fn mock_call_proxy() {
    ...
    let proxy_address = deploy_contract(&mut blockifier_state, &mut cheatnet_state, "MockCheckerProxy", &[]);
...
fn mock_call_library_call_no_effect() {
    ...
    let contract_address = deploy(&mut blockifier_state, &mut cheatnet_state, &class_hash, &[Felt252::from(420)])
        .unwrap()
        .contract_address;

    let lib_call_address = deploy_contract(&mut blockifier_state, &mut cheatnet_state, "MockCheckerLibCall", &[]);
...
fn mock_call_before_deployment() {
    ...
    let precalculated_address = cheatnet_state.precalculate_address(&class_hash, &[Felt252::from(420)]);
...
fn mock_call_in_constructor() {
    ...
    let class_hash = blockifier_state.declare(&contract_name, &contracts).unwrap();
    let balance_contract_address = deploy(&mut blockifier_state, &mut cheatnet_state, &class_hash, &[]).unwrap().contract_address;
    ...
    let class_hash = blockifier_state.declare(&contract_name, &contracts).unwrap();
    let contract_address = deploy(&mut blockifier_state, &mut cheatnet_state, &class_hash, &[balance_contract_address.to_felt252()])
        .unwrap()
        .contract_address;
...
fn mock_call_two_methods() {
    ...
    let output = call_contract(&mut blockifier_state, &mut cheatnet_state, &contract_address, &selector1, &[]).unwrap();

    assert_success!(output, ret_data);

    let output = call_contract(&mut blockifier_state, &mut cheatnet_state, &contract_address, &selector2, &[]).unwrap();
