...
fn roll_proxy() {
    ...
    let proxy_address = deploy_contract(&mut blockifier_state, &mut cheatnet_state, "RollCheckerProxy", &[]);
...
fn roll_library_call() {
    ...
    let lib_call_address = deploy_contract(&mut blockifier_state, &mut cheatnet_state, "RollCheckerLibCall", &[]);
