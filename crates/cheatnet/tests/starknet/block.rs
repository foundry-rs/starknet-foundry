...
fn check_block(
    ...
    let output = call_contract(blockifier_state, cheatnet_state, contract_address, &write_block, &[]).unwrap();

    assert_success!(output, vec![]);

    let output = call_contract(blockifier_state, cheatnet_state, contract_address, &read_block_number, &[]).unwrap();

    let block_number = &recover_data(output)[0];

    let output = call_contract(blockifier_state, cheatnet_state, contract_address, &read_block_timestamp, &[]).unwrap();

    let block_timestamp = &recover_data(output)[0];

    let output = call_contract(blockifier_state, cheatnet_state, contract_address, &read_sequencer_address, &[]).unwrap();

    let sequencer_address = &recover_data(output)[0];

    let output = call_contract(blockifier_state, cheatnet_state, contract_address, &read_block_hash, &[]).unwrap();
...
fn block_does_not_decrease() {
    ...
    let contract_address = deploy_contract(&mut blockifier_state, &mut cheatnet_state, "Blocker", &[]);

    let (old_block_number, old_block_timestamp, old_sequencer_address, old_block_hash) =
        check_block(&mut blockifier_state, &mut cheatnet_state, &contract_address);
