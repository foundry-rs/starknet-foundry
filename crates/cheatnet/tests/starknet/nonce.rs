...
fn check_nonce(blockifier_state: &mut BlockifierState, cheatnet_state: &mut CheatnetState, contract_address: &ContractAddress) -> Felt252 {
    let write_nonce = felt_selector_from_name("write_nonce");
    let read_nonce = felt_selector_from_name("read_nonce");

    let output = call_contract(blockifier_state, cheatnet_state, contract_address, &write_nonce, &[]).unwrap();

    assert_success!(output, vec![]);

    let output = call_contract(blockifier_state, cheatnet_state, contract_address, &read_nonce, &[]).unwrap();
...
fn nonce_transactions() {
    ...
    let old_nonce = check_nonce(&mut blockifier_state, &mut cheatnet_state, &contract_address);
    let new_nonce = check_nonce(&mut blockifier_state,&mut cheatnet_state, &contract_address);
...
fn nonce_declare_deploy() {
    ...
    let nonce1 = check_nonce(&mut blockifier_state, &mut cheatnet_state, &contract_address);

    let class_hash = blockifier_state.declare(&contract_name, &contracts).unwrap();

    let nonce2 = check_nonce(&mut blockifier_state, &mut cheatnet_state, &contract_address);

    deploy(&mut blockifier_state, &mut cheatnet_state, &class_hash, &[]).unwrap();

    let nonce3 = check_nonce(&mut blockifier_state, &mut cheatnet_state, &contract_address);
