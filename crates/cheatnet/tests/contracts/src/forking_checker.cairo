...
trait IForkingChecker<TContractState> {
    fn get_balance_call_contract(ref self: TContractState, contract_address: ContractAddress) -> felt252;
...
#[starknet::contract]
mod ForkingChecker {
    use super::{IHelloStarknetDispatcherTrait, IHelloStarknetDispatcher, IHelloStarknetLibraryDispatcher};
    ...
    fn constructor(ref self: ContractState, contract_address: Option<ContractAddress>) {
        if contract_address.is_some() {
            let hello_starknet = IHelloStarknetDispatcher { contract_address: contract_address.unwrap() };
    ...
    impl IForkingCheckerImpl of super::IForkingChecker<ContractState> {
        fn get_balance_call_contract(ref self: ContractState, contract_address: ContractAddress) -> felt252 {
