...
fn get_starknet_artifacts_paths(
    target_dir: &Utf8Path,
    target_name: &str,
    current_profile: &str,
    use_test_target_contracts: bool,
) -> Option<Vec<ContractArtifactData>> {
    ...
    let starknet_artifacts_file_name = |test_type: Option<&str>| -> Option<ContractArtifactData> {
        if let Some(test_type) = test_type {
            let path = format!("{target_name}_{test_type}test.test.starknet_artifacts.json");
    ...
    if use_test_target_contracts {
        Some(
            ["integration", "unit"]
                .iter()
                .filter_map(|test_type| starknet_artifacts_file_name(Some(test_type)))
                .collect(),
        )
    } else {
        Some(vec![starknet_artifacts_file_name(None)?])
    }
}
...
pub fn get_contracts_artifacts_and_source_sierra_paths(
    ...
    let maybe_contracts_path = get_starknet_artifacts_paths(
        &target_dir,
        &target_name,
    ...
    let map = match maybe_contracts_path {
        Some(contracts_path) => load_contracts_artifacts(&contracts_path)?,
...
mod tests {
    ...
    fn get_starknet_artifacts_path_for_standard_build() {
        ...
        let path = get_starknet_artifacts_paths(
    ...
    fn get_starknet_artifacts_path_for_test_build() {
        ...
        let path = get_starknet_artifacts_paths(
            &Utf8PathBuf::from_path_buf(temp.join("target")).unwrap(),
            "basic_package",
            "dev",
            true,
        )
