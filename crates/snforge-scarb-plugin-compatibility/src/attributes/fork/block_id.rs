    ...
    fn as_cairo_expression(&self) -> String {
        match self {
            Self::Hash(hash) => format!(
                "snforge_std_compatibility::_internals::config_types::BlockId::BlockHash({})",
                hash.as_cairo_expression()
            ),
            Self::Number(number) => format!(
                "snforge_std_compatibility::_internals::config_types::BlockId::BlockNumber({})",
                number.as_cairo_expression()
            ),
            Self::Tag => "snforge_std_compatibility::_internals::config_types::BlockId::BlockTag".to_string(),
        }
    }
}
