    ...
    fn as_cairo_expression(&self) -> String {
        match self {
            Self::Felt(felt) => {
                ...
                format!("snforge_std_compatibility::_internals::config_types::Expected::ShortString({string})")
            }
            Self::ByteArray(string) => {
                let string = string.as_cairo_expression();

                format!(r"snforge_std_compatibility::_internals::config_types::Expected::ByteArray({string})")
            }
            Self::Array(strings) => {
                let arr = strings.as_cairo_expression();

                format!("snforge_std_compatibility::_internals::config_types::Expected::Array({arr})")
            }
            Self::Any => "snforge_std_compatibility::_internals::config_types::Expected::Any".to_string(),
        }
    }
}
