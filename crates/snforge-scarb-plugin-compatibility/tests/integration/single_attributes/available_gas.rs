...
use indoc::formatdoc;
use snforge_scarb_plugin_compatibility::attributes::available_gas::available_gas;
    ...
    assert_output(
        ...
        "
            fn empty_fn() {
                if snforge_std_compatibility::_internals::is_config_run() {
                    let mut data = array![];
                    snforge_std_compatibility::_internals::config_types::AvailableGasConfig::MaxGas(0x3)
                    .serialize(ref data);
                    starknet::testing::cheatcode::<'set_config_available_gas'>(data.span());
                    return;
                }
