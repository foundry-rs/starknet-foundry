...
// pub struct Workspace(pub HashMap<PackageId, Vec<TestTargetWithConfig>>);
...
//                 .into_iter()
//                 .map(|raw| {
//                     let tt_with_config = test_target_with_config(raw, &args.tracked_resource)?;
//                     Ok(tt_with_config)
//                 })
//                 .collect::<Result<Vec<TestTargetWithConfig>>>()?;
