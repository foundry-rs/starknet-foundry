...
pub fn clean(args: CleanArgs) -> Result<()> {
    ...
    let packages_root: Vec<PathBuf> = scarb_metadata.packages.into_iter().map(|x| x.root.into()).collect();
    let cache_dir: PathBuf = scarb_metadata.workspace.root.join(".snfoundry_cache").into();
    let trace_dir: PathBuf = scarb_metadata.workspace.root.join(".snfoundry_trace").into();
    ...
    for package_root in packages_root {
        ...
        if coverage_dir.exists() {
            fs::remove_dir_all(&coverage_dir)
                .with_context(|| format!("Failed to remove coverage directory: {}", coverage_dir.display()))?;
    ...
    for package_root in packages_root {
        ...
        if profile_dir.exists() {
            fs::remove_dir_all(&profile_dir)
                .with_context(|| format!("Failed to remove profile directory: {}", profile_dir.display()))?;
...
fn clean_cache(cache_dir: &PathBuf, trace_dir: &PathBuf) -> Result<()> {
    ...
    if trace_dir.exists() {
        fs::remove_dir_all(&trace_dir)
            .with_context(|| format!("Failed to remove trace directory: {}", trace_dir.display()))?;
