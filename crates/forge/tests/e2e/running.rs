...
use std::fs;
use test_utils::{get_snforge_std_entry, use_snforge_std_compatibility};
...
fn simple_package_with_git_dependency() {
    ...
    let snforge_std = if use_snforge_std_compatibility() {
        format!(
            r#"snforge_std_compatibility = {{ git = "https://github.com/{remote_url}", branch = "{branch}" }}"#
        )
    } else {
        format!(
