...
fn new_scarb_new_macros() {
    ...
    assert_stdout_contains(
        ...
        indoc! {r"
            ...
            [PASS] abc_integrationtest::test_contract::test_cannot_increase_balance_with_zero_value (l1_gas: ~0, l1_data_gas: ~96, l2_gas: ~360000)
            [PASS] abc_integrationtest::test_contract::test_increase_balance (l1_gas: ~0, l1_data_gas: ~192, l2_gas: ~480000)
...
#[test]
#[cfg_attr(feature = "skip_plugin_checks", ignore)]
fn new_scarb_compatibility_macros() {
    let temp = tempdir_with_tool_versions().unwrap();
    runner(&temp)
        .env("DEV_DISABLE_SNFORGE_STD_DEPENDENCY", "true")
        .args(["new", "abc"])
        .assert()
        .success();
    let snforge_std = Utf8PathBuf::from("../../snforge_std_compatibility")
        .canonicalize_utf8()
        .unwrap();
    ScarbCommand::new()
        .current_dir(temp.path().join("abc"))
    ...
    assert_stdout_contains(
        output,
        formatdoc! {r"
            error: the required Cairo version of package snforge_std_compatibility is not compatible with current version
        ...
        ", scarb_version.major, scarb_version.minor, scarb_version.patch},
    );
}
...
fn old_scarb_new_macros() {
    ...
    assert_stdout_contains(
        output,
        indoc! {r"
            [ERROR] On Scarb versions < 2.12.0, the `snforge_std` package must be replaced with `snforge_std_compatibility`. Please update it in Scarb.toml
        "},
    );
}
...
#[test]
#[cfg_attr(feature = "skip_plugin_checks", ignore)]
fn old_scarb_compatibility_macros() {
    let temp = tempdir_with_tool_versions().unwrap();
    Command::new("asdf")
        .current_dir(&temp)
        .args(["set", "scarb", "2.10.1"])
