...
fn advanced_types() {
    let test = test_case!(
        ...
        fn advanced_types() {
            ...
            let contract = declare("ERC20").unwrap().success_contract_class();
...
fn serding() {
    let test = test_case!(
        ...
        #[test]
        fn serding() {
            let contract = declare("Serding").unwrap().success_contract_class();
            let (contract_address, _) = contract.deploy( @ArrayTrait::new()).unwrap();
...
fn proxy_storage() {
    let test = test_case!(
        ...
        fn deploy_contract(name: ByteArray) -> ContractAddress {
            let contract = declare(name).unwrap().success_contract_class();
            let (contract_address, _) = contract.deploy(@ArrayTrait::new()).unwrap();
            contract_address
        }
...
fn nonexistent_libcall_function() {
    let test = test_case!(
        ...
        #[test]
        fn nonexistent_libcall_function() {
            let class = declare("Contract").unwrap().success_contract_class().clone();
