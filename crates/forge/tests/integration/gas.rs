...
fn l1_message_cost_for_proxy_cairo_steps() {
    ...
    assert_passed(&result);
    // 5167 * 0.0025 = 12.9175 ~ 13 = gas cost of steps
    // l = number of class hash updates
    // n = unique contracts updated
    // So, as per formula:
    // n(2) * 2 * 32 = 128
...
fn events_contract_cost_cairo_steps() {
    ...
    let result = run_test_case(&test, ForgeTrackedResource::CairoSteps);
    assert_passed(&result);
    // (4048 steps + 14 memory holes) * 0.0025 = 10.155 ~ 11 = gas cost of steps
    // 96 = gas cost of onchain data (deploy cost)
    // 768_000 gas for 50 events
    //      512_000 = 10_240 * 50 (gas from 50 event keys)
    //      256_000 = 5120 * 50 (gas from 50 event data)
    // 0 l1_gas + 96 l1_data_gas + ((11) * (100 / 0.0025) + 768_000) l2 gas
    assert_gas(
        &result,
        "event_emission_cost",
        GasVector {
...
fn deploy_syscall_cost_sierra_gas() {
    ...
    // 448734 = reported consumed sierra gas
    // 0 l1_gas + 96 l1_data_gas + (20000 + 147660 + 448734) l2 gas
    assert_gas(
        &result,
        "deploy_syscall_cost",
        GasVector {
...
fn snforge_std_deploy_cost_sierra_gas() {
    ...
    // 462384 = reported consumed sierra gas
    // 0 l1_gas + 96 l1_data_gas + (20000 + 147660 + 462384) l2 gas
    assert_gas(
        &result,
        "deploy_cost",
        GasVector {
...
fn contract_keccak_cost_sierra_gas() {
    ...
    // 1107975 = reported consumed sierra gas
    // 0 l1_gas + 96 l1_data_gas + (142810 + 50000 + 87650 + 1107975) l2 gas
    assert_gas(
        &result,
        "contract_keccak_cost",
        GasVector {
...
fn contract_range_check_cost_sierra_gas() {
    ...
    // 110480 = reported consumed sierra gas
    // 0 l1_gas + 96 l1_data_gas + (142810 + 87650 + 110480) l2 gas
    assert_gas(
        &result,
        "contract_range_check_cost",
        GasVector {
...
fn storage_write_cost_sierra_gas() {
    ...
    // 48300 = reported consumed sierra gas
    // 0 l1_gas + (96 + 64 + 32) l1_data_gas + (142810 + 87650 + 10000 + 48300) l2 gas
    assert_gas(
        &result,
        "storage_write_cost",
        GasVector {
...
fn multiple_storage_writes_cost_sierra_gas() {
    ...
    //      -> 1 range check costs 70
    //      -> the minimum total cost is `syscall_base_gas_cost`, which is pre-charged by the compiler (atm it is 100 * 100)
    // 58110 = reported consumed sierra gas
    // 0 l1_gas + (64 + 64 + 32 + 32) l1_data_gas + (142810 + 175300 + 20000 + 58110) l2 gas
    assert_gas(
        &result,
        "multiple_storage_writes_cost",
        GasVector {
...
fn l1_message_cost_sierra_gas() {
    ...
    //      -> 1 range check costs 70
    // 47960 = reported consumed sierra gas
    // 29524 l1_gas + 96 l1_data_gas + (142810 + 87650 + 14170 + 47960) l2 gas
    assert_gas(
        &result,
        "l1_message_cost",
        GasVector {
...
fn l1_message_cost_for_proxy_sierra_gas() {
    ...
    // 88000 = reported consumed sierra gas
    // 29524 l1_gas + (128 + 64) l1_data_gas + (285620 + 175300 + 14170 + 88000) l2 gas
    assert_gas(
        &result,
        "l1_message_cost_for_proxy",
        GasVector {
...
fn events_contract_cost_sierra_gas() {
    ...
    // 142810 = cost of 1 deploy syscall (see `deploy_syscall_cost_sierra_gas` test)
    // 87650 = cost of 1 call contract syscall (see `contract_keccak_cost_sierra_gas` test)
    // 208040 = reported consumed sierra gas
    // 0 l1_gas + 96 l1_data_gas + (512000 + 256000 + 10000 + 142810 + 87650 + 208040) l2 gas
    assert_gas(
        &result,
        "event_emission_cost",
        GasVector {
...
fn nested_call_cost_sierra_gas() {
    ...
    // 142810 = cost of 1 deploy syscall (see `deploy_syscall_cost_sierra_gas` test)
    // 87650 = cost of 1 call contract syscall (see `contract_keccak_cost_sierra_gas` test)
    // 595082 = reported consumed sierra gas
    // 0 l1_gas + 288 l1_data_gas + (512000 + 256000 + 10000 + 3 * 142810 + 2 * 87650 + 595082) l2 gas
    assert_gas(
        &result,
        "test_call_other_contract",
        GasVector {
...
fn nested_call_cost_in_forked_contract_sierra_gas() {
    ...
    // 142810 = cost of 1 deploy syscall (see `deploy_syscall_cost_sierra_gas` test)
    // 87650 = cost of 1 call contract syscall (see `contract_keccak_cost_sierra_gas` test)
    // 581442 = reported consumed sierra gas
    // 0 l1_gas + 192 l1_data_gas + (512000 + 256000 + 10000 + 2 * 142810 + 2 * 87650 + 581442) l2 gas
    assert_gas(
        &result,
        "test_call_other_contract_fork",
        GasVector {
