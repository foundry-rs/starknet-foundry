...
fn storage_write_cost_cairo_steps() {
    ...
    // 0 l1_gas + (96 + 64 + 32) l1_data_gas + 4 * (100 / 0.0025) l2 gas
    assert_gas(
        &result,
        "storage_write_cost",
        GasVector {
...
fn declare_cost_is_omitted_sierra_gas() {
    ...
    assert_passed(&result);
    // 34240 = reported consumed sierra gas
    // 0 l1_gas + 0 l1_data_gas + 34240 l2 gas
    assert_gas(
        &result,
        "declare_cost_is_omitted",
        GasVector {
...
fn deploy_syscall_cost_sierra_gas() {
    ...
    // 647904 = reported consumed sierra gas
    // 0 l1_gas + 96 l1_data_gas + (20000 + 147660 + 647904) l2 gas
    assert_gas(
        &result,
        "deploy_syscall_cost",
        GasVector {
...
fn snforge_std_deploy_cost_sierra_gas() {
    ...
    // 58690 = reported consumed sierra gas
    // 0 l1_gas + 96 l1_data_gas + (20000 + 147660 + 58690) l2 gas
    assert_gas(
        &result,
        "deploy_cost",
        GasVector {
...
fn keccak_cost_sierra_gas() {
    ...
    //      -> 1 keccak syscall costs 100 cairo steps
    // 246537 = reported consumed sierra gas
    // 0 l1_gas + 0 l1_data_gas + (10000 + 246537) l2 gas
    assert_gas(
        &result,
        "keccak_cost",
        GasVector {
...
fn contract_keccak_cost_sierra_gas() {
    ...
    // 209920 = reported consumed sierra gas
    // 0 l1_gas + 96 l1_data_gas + (50000 + 87650 + 209920) l2 gas
    assert_gas(
        &result,
        "contract_keccak_cost",
        GasVector {
...
fn contract_range_check_cost_sierra_gas() {
    ...
    // 66710 = reported consumed sierra gas
    // 0 l1_gas + 96 l1_data_gas + (142810 + 87650 + 66710) l2 gas
    assert_gas(
        &result,
        "contract_range_check_cost",
        GasVector {
...
fn storage_write_cost_sierra_gas() {
    ...
    // 67110 = reported consumed sierra gas
    // 0 l1_gas + (96 + 64 + 32) l1_data_gas + (142810 + 87650 + 10000 + 67110) l2 gas
    assert_gas(
        &result,
        "storage_write_cost",
        GasVector {
...
fn multiple_storage_writes_cost_sierra_gas() {
    ...
    //      -> 1 range check costs 70
    //      -> the minimum total cost is `syscall_base_gas_cost`, which is pre-charged by the compiler (atm it is 100 * 100)
    // 78810 = reported consumed sierra gas
    // 0 l1_gas + (64 + 64 + 32 + 32) l1_data_gas + (142810 + 175300 + 20000 + 78810) l2 gas
    assert_gas(
        &result,
        "multiple_storage_writes_cost",
        GasVector {
...
fn l1_message_cost_sierra_gas() {
    ...
    //      -> 1 range check costs 70
    // 66710 = reported consumed sierra gas
    // 29524 l1_gas + 96 l1_data_gas + (142810 + 87650 + 14170 + 66710) l2 gas
    assert_gas(
        &result,
        "l1_message_cost",
        GasVector {
...
fn l1_message_cost_for_proxy_sierra_gas() {
    ...
    // 104910 = reported consumed sierra gas
    // 29524 l1_gas + (128 + 64) l1_data_gas + (285620 + 175300 + 14170 + 104910) l2 gas
    assert_gas(
        &result,
        "l1_message_cost_for_proxy",
        GasVector {
...
fn events_cost_sierra_gas() {
    ...
    //      -> 1 range check costs 70
    //      -> the minimum total cost is `syscall_base_gas_cost`, which is pre-charged by the compiler (atm it is 100 * 100)
    // 207450 = reported consumed sierra gas
    // 0 l1_gas + 0 l1_data_gas + (512000 + 256000 + 10000 + 207450) l2 gas
    assert_gas(
        &result,
        "events_cost",
        GasVector {
...
fn events_contract_cost_sierra_gas() {
    ...
    // 87650 = cost of 1 call contract syscall (see `contract_keccak_cost_sierra_gas` test)
    // 67110 = reported consumed sierra gas
    // 0 l1_gas + 96 l1_data_gas + (512000 + 256000 + 10000 + 142810 + 87650 + 67110) l2 gas
    assert_gas(
        &result,
        "event_emission_cost",
        GasVector {
...
fn nested_call_cost_sierra_gas() {
    ...
    // 142810 = cost of 1 deploy syscall (see `deploy_syscall_cost_sierra_gas` test)
    // 87650 = cost of 1 call contract syscall (see `contract_keccak_cost_sierra_gas` test)
    // 362485 = reported consumed sierra gas
    // 0 l1_gas + 288 l1_data_gas + (512000 + 256000 + 10000 + 3 * 142810 + 2 * 87650 + 362485) l2 gas
    assert_gas(
        &result,
        "test_call_other_contract",
        GasVector {
...
fn nested_call_cost_in_forked_contract_sierra_gas() {
    ...
    // 142810 = cost of 1 deploy syscall (see `deploy_syscall_cost_sierra_gas` test)
    // 87650 = cost of 1 call contract syscall (see `contract_keccak_cost_sierra_gas` test)
    // 324595 = reported consumed sierra gas
    // 0 l1_gas + 192 l1_data_gas + (512000 + 256000 + 10000 + 2 * 142810 + 2 * 87650 + 324595) l2 gas
    assert_gas(
        &result,
        "test_call_other_contract_fork",
        GasVector {
