...
fn override_entrypoint() {
    let test = test_case!(
        indoc!(
            ...
            #[test]
            fn override_entrypoint() {
                let contract = declare("ReplaceBytecodeA").unwrap().success_contract_class();
                let contract_b_class = declare("ReplaceBytecodeB").unwrap().success_contract_class().class_hash.clone();
...
fn libcall_in_cheated() {
    let test = test_case!(
        indoc!(
            ...
            fn override_entrypoint() {
                let contract = declare("ReplaceBytecodeA").unwrap().success_contract_class();
                let contract_b_class = declare("ReplaceBytecodeB").unwrap().success_contract_class().clone().class_hash;
                let lib = declare("Lib").unwrap().success_contract_class().clone().class_hash;
                let (contract_address, _) = contract.deploy(@ArrayTrait::new()).unwrap();
                let dispatcher = IReplaceBytecodeDispatcher { contract_address };
...
fn contract_not_deployed() {
    let test = test_case!(
        indoc!(
            ...
            #[test]
            fn contract_not_deployed() {
                let class_hash = declare("ReplaceBytecodeA").unwrap().success_contract_class().clone().class_hash;
