    ...
    let test = test_case!(
        indoc!(
            ...
            fn deploy_cheat_block_timestamp_checker()  -> ICheatBlockTimestampCheckerDispatcher {
                let contract = declare("CheatBlockTimestampChecker").unwrap().success_contract_class();
...
fn cheat_block_timestamp_with_span() {
    let test = test_case!(
        indoc!(
            ...
            fn deploy_cheat_block_timestamp_checker() -> ICheatBlockTimestampCheckerDispatcher {
                let (contract_address, _) = declare("CheatBlockTimestampChecker").unwrap().success_contract_class().deploy(@ArrayTrait::new()).unwrap();
                ICheatBlockTimestampCheckerDispatcher { contract_address }
            }
