    ...
    let test = test_case!(
        indoc!(
            ...
            fn test_meta_tx_v0_with_cheat_block_number() {
                ...
                let old_block_number = meta_dispatcher.get_block_number();

                let cheated_block_number = 999;
                start_cheat_block_number(meta_address, cheated_block_number);

                let signature = ArrayTrait::new();

                let meta_result = meta_dispatcher.execute_meta_tx_get_block_number(meta_address, signature.span());

                assert(meta_result == 999, 'Should see cheated block');

                stop_cheat_block_number(meta_address);

                let new_block_number = meta_dispatcher.get_block_number();
    ...
    let test = test_case!(
        indoc!(
            ...
            fn test_meta_tx_v0_with_cheat_block_hash() {
                ...
                let old_block_hash = meta_dispatcher.get_block_hash(block_number);

                let cheated_hash = 555;
                start_cheat_block_hash(meta_address, block_number, cheated_hash);

                let signature = ArrayTrait::new();

                let meta_result = meta_dispatcher.execute_meta_tx_get_block_hash(meta_address, block_number, signature.span());

                assert(meta_result == 555, 'Should see cheated hash');

                stop_cheat_block_hash(meta_address, block_number);

                let new_block_hash = meta_dispatcher.get_block_hash(block_number);
    ...
    let test = test_case!(
        indoc!(
            ...
            fn test_meta_tx_v0_with_cheat_sequencer_address() {
                ...
                let old_sequencer = meta_dispatcher.get_sequencer_address();

                let cheated_address: ContractAddress = 777.try_into().unwrap();
                start_cheat_sequencer_address(meta_address, cheated_address);

                let signature = ArrayTrait::new();

                let meta_result = meta_dispatcher.execute_meta_tx_get_sequencer_address(meta_address, signature.span());

                assert(meta_result == 777, 'Should see cheated seq');

                stop_cheat_sequencer_address(meta_address);

                let new_sequencer = meta_dispatcher.get_sequencer_address();
    ...
    let test = test_case!(
        indoc!(
            ...
            fn test_meta_tx_v0_with_cheat_block_timestamp() {
                ...
                let old_timestamp = meta_dispatcher.get_block_timestamp();

                let cheated_timestamp = 1234567890;
                start_cheat_block_timestamp(meta_address, cheated_timestamp);

                let signature = ArrayTrait::new();

                let meta_result = meta_dispatcher.execute_meta_tx_get_block_timestamp(meta_address, signature.span());

                assert(meta_result == 1234567890, 'Should see cheated time');

                stop_cheat_block_timestamp(meta_address);

                let new_timestamp = meta_dispatcher.get_block_timestamp();
