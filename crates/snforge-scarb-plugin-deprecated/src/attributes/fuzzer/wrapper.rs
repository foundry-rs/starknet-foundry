...
fn fuzzer_wrapper_internal(
    ...
    let fuzzer_assignments = extract_and_transform_params(
        ...
        |param| {
            format!(
                r"
                let {}{} = snforge_std_deprecated::fuzzable::Fuzzable::generate();
                snforge_std_deprecated::_internals::save_fuzzer_arg(@{});
                ",
    ...
    let blank_values_for_config_run = extract_and_transform_params(
        db,
        func,
        |_param| "snforge_std_deprecated::fuzzable::Fuzzable::blank()".to_string(),
        ", ",
    );
