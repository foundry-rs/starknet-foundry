...
{{fee_token}} fee token is not supported for {{deployment}} deployment.
...
impl Deploy {
    pub fn validate(&self) -> Result<()> {
        match (&self.version, &self.fee_args.fee_token) {
            (Some(AccountDeployVersion::V3), Some(FeeToken::Eth)) => Err(anyhow!(ERROR_TEMPLATE
                .replace("{{deployment}}", "v3")
                .replace("{{fee_token}}", "eth"))),
            (Some(AccountDeployVersion::V1), Some(FeeToken::Strk)) => Err(anyhow!(ERROR_TEMPLATE
                .replace("{{deployment}}", "v1")
                .replace("{{fee_token}}", "strk"))),
