...
pub mod db;
use super::explorer::VerificationInterface;
    ...
    fn gather_packages(
            ...
            for dependency in &package.dependencies {
                ...
                if url.scheme().starts_with("path") {
                    let path = url.to_file_path().map_err(|_| {
                        dependency_path(name.clone(), dependency.source.repr.clone())
                    })?;
        ...
        for (name, manifest) in out_of_workspace_dependencies {
            let new_meta = MetadataCommand::new()
                ...
                .exec()
                .map_err(|_| {
                    metadata_error(name.clone(), manifest.to_string_lossy().to_string())
                })?;
            self.gather_packages(&new_meta, packages)?;
        }
...
impl VerificationInterface for Voyager {
    ...
    async fn verify(
        ...
        self.gather_packages(&self.metadata, &mut packages)?;

        let mut sources: Vec<Utf8PathBuf> = vec![];
        for package in &packages {
            let mut package_sources = Voyager::package_sources(package)?;
            sources.append(&mut package_sources);
        }

        let prefix = Voyager::biggest_common_prefix(&sources, &self.metadata.workspace.root);
        ...
        let selected = (if workspace_packages.len() > 1 {
            match package {
                Some(package_name) => {
                    if let Some(p) = workspace_packages.into_iter().find(|p| p.name == package_name) {
                        Ok(p)
                    } else {
                        Err(anyhow!("Package {} not found in scarb metadata", package_name))
                    }
                },
                None => Err(anyhow!("More than one package found in scarb metadata - specify package using --package flag")),
            }
        } else {
            if let Some(p) = workspace_packages.pop() {
                Ok(p)
            } else {
