...
pub struct ContractAddressArgs {
    ...
    /// Deployer account address (hex, decimal, or @alias from snfoundry.toml), used to modify the salt when `--unique` is set. Defaults to zero.
    #[arg(long, default_value = "0")]
    pub deployer_address: DeployerAccountAddress,
...
pub async fn get_contract_address(
    ...
    let contract_address = get_udc_deployed_address(
        salt,
        class_hash,
        &udc_uniqueness(args.common.unique, deployer_address),
        &calldata,
    );
