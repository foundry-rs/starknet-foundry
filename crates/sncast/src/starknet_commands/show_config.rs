...
pub async fn show_config(
    ...
    let rpc_url = show.rpc.url.clone().or(cast_config.url().clone());
    let account = Some(cast_config.account().to_string());
    let mut accounts_file_path = Some(cast_config.accounts_file().clone());
    let keystore = cast_config.keystore().clone();
    if keystore.is_some() {
    ...
    let show_explorer_links = cast_config.show_explorer_links();
