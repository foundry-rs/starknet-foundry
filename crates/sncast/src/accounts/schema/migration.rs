...
pub fn v1_to_domain(file: v1::AccountsFile) -> Result<AccountRegistry, AccountsError> {
...
pub fn v2_to_domain(file: v2::AccountsFile) -> Result<AccountRegistry, AccountsError> {
    convert_networks(file.accounts, |network, account_name, account| {
        let signer = match account.signer {
            ...
            v2::Signer::Ledger { derivation_path } => {
                let derivation_path = parse_derivation_path(&derivation_path).map_err(|error| {
                    AccountsError::Migration {
                        network: network.to_owned(),
                        account: account_name.to_owned(),
                        message: error.to_string(),
...
pub fn domain_to_v2(registry: &AccountRegistry) -> v2::AccountsFile {
