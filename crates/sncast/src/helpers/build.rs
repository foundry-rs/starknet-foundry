...
use scarb_artifacts::{get_contracts_map, target_dir_for_package, StarknetContractArtifacts, ScarbCommand};
...
pub fn build(
    ...
    command.arg("build").manifest_path(&package.manifest_path).packages_filter(filter);
    if build_config.json {
        command.json();
    }
    command.run().context("Failed to build contracts with Scarb")?;
