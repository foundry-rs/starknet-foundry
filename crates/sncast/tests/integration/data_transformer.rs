...
async fn test_function_not_found() {
    ...
    output.unwrap_err().assert_contains(
...
#[tokio::test]
async fn test_happy_case_u256_function_cairo_expressions_input() -> anyhow::Result<()> {
    let contract_class = CLASS.get_or_init(init_class).await.to_owned();

    let input = vec![U256::MAX.to_string()];
...
async fn test_happy_case_signed_function_cairo_expressions_input() -> anyhow::Result<()> {
    ...
    let expected_output = vec![Felt::from(-273i16)];
...
// Problem: Although transformer fails to process the given input as `i32`, itthen succeeds to interpret it as `felt252`
...
async fn test_signed_fn_overflow_with_type_suffix() {
    let simulated_cli_input = vec![format!("{}_i32", i32::MAX as u64 + 1)];

    let contract_class = CLASS.get_or_init(init_class).await.to_owned();
    let selector = get_selector_from_name("signed_fn").unwrap();

    let result = transform(&simulated_cli_input, contract_class, &selector);
...
async fn test_happy_case_unsigned_function_cairo_expressions_input() -> anyhow::Result<()> {
    ...
    let expected_output = vec![Felt::from(u32::MAX)];
...
async fn test_happy_case_tuple_function_with_nested_struct_cairo_expression_input(
    ...
    let expected_output: Vec<Felt> = vec![123, 234, 2, 345, 456]
        .into_iter()
        .map(Felt::from)
        .collect();
...
async fn test_happy_case_simple_struct_function_cairo_expression_input() -> anyhow::Result<()> {
    ...
    let expected_output = vec![Felt::from_hex_unchecked("0x12")];
...
async fn test_happy_case_enum_function_empty_variant_cairo_expression_input() -> anyhow::Result<()>
{
    ...
    let expected_output = vec![Felt::ZERO];
...
async fn test_happy_case_complex_struct_function_cairo_expression_input() -> anyhow::Result<()> {
    ...
    let data = concat!(
        r#"ComplexStruct {"#,
        r#"    a: NestedStructWithField {"#,
        r#"        a: SimpleStruct { a: 1 },"#,
        r#"        b: 2"#,
        r#"    },"#,
        r#"    b: 3, c: 4, d: 5,"#,
        r#"    e: Enum::Two(6),"#,
...
async fn test_happy_case_complex_struct_function_serialized_input() -> anyhow::Result<()> {
    ...
    let input = felts.clone().into_iter().map(String::from).collect_vec();
...
async fn test_external_struct_function_ambiguous_struct_name_cairo_expression_input() {
    ...
    let input = vec![
        String::from("BitArray { bit: 23 }"),
        String::from("BitArray { data: array![0], current: 1, read_pos: 2, write_pos: 3 }"),
    ];
...
async fn test_happy_case_external_struct_function_cairo_expression_input() -> anyhow::Result<()> {
    ...
    let input = vec![
    	String::from("data_transformer_contract::BitArray { bit: 23 }"),
     	String::from("alexandria_data_structures::bit_array::BitArray { data: array![0], current: 1, read_pos: 2, write_pos: 3 }")
    ];
...
async fn test_happy_case_external_struct_function_serialized_input() -> anyhow::Result<()> {
    ...
    let felts = vec!["0x17", "0x1", "0x0", "0x1", "0x2", "0x3"];
...
async fn test_external_struct_function_invalid_path_to_external_struct() {
    ...
    let input = vec![
        String::from("something::BitArray { bit: 23 }"),
        String::from("BitArray { data: array![0], current: 1, read_pos: 2, write_pos: 3 }"),
    ];
...
async fn test_happy_case_contract_constructor() -> anyhow::Result<()> {
    ...
    let expected_output = vec![Felt::from_hex_unchecked("0x123")];
