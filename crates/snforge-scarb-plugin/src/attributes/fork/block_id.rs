    ...
    fn as_cairo_expression(&self) -> TokenStream {
        match self {
            Self::Hash(hash) => {
                ...
                quote!(snforge_std::_config_types::BlockId::BlockHash(#block_hash))
            }
            Self::Number(number) => {
                let block_number = number.as_cairo_expression();
                quote!(snforge_std::_config_types::BlockId::BlockNumber(#block_number))
            }
            Self::Tag => quote!(snforge_std::_config_types::BlockId::BlockTag),
        }
    }
}
