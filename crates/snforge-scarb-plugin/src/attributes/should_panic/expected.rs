    ...
    fn as_cairo_expression(&self) -> TokenStream {
        match self {
            Self::Felt(felt) => {
                ...
                quote!(snforge_std::_config_types::Expected::ShortString(#string))
            }
            Self::ByteArray(string) => {
                let string = string.as_cairo_expression();

                quote!(snforge_std::_config_types::Expected::ByteArray(#string))
            }
            Self::Array(strings) => {
                let arr = strings.as_cairo_expression();

                quote!(snforge_std::_config_types::Expected::Array(#arr))
            }
            Self::Any => quote!(snforge_std::_config_types::Expected::Any),
        }
    }
}
