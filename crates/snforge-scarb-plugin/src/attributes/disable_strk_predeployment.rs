...
pub struct DisabledStrkPredeploymentCollector;

impl AttributeInfo for DisabledStrkPredeploymentCollector {
    const ATTR_NAME: &'static str = "disable_strk_predeployment";
}

impl AttributeTypeData for DisabledStrkPredeploymentCollector {
...
pub fn disable_strk_predeployment(args: TokenStream, item: TokenStream) -> ProcMacroResult {
    extend_with_config_cheatcodes::<DisabledStrkPredeploymentCollector>(args, item)
}
